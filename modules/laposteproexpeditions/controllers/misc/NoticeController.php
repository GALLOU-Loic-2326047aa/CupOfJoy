...
namespace LaPoste\LaPosteProExpeditionsPrestashop\Controllers\Misc;

if (!defined('_PS_VERSION_')) {
    exit;
}

use LaPoste\LaPosteProExpeditionsPrestashop\Notice\CustomNotice;
...
class NoticeController
{
     ...
     */
    public static function getNoticeInstances($controller)
    {
        ...
        if (is_array($notices)) {
            foreach ($notices as $notice) {
                $key = $notice['key'];
                $classname = 'LaPoste\LaPosteProExpeditionsPrestashop\Notice\\';
                $noticeInstance = null;
                if (!in_array($key, self::$coreNotices, true)) {
                    $value = EncodeUtil::decode($notice['value']);
                    if (false !== $value) {
                        $noticeInstance = new CustomNotice($key, $notice['id_shop_group'], $notice['id_shop'], $value);
                    } else {
                    ...
                    if (class_exists($classname, true)) {
                        ...
                        if (false !== $value && null !== $value) {
                            $noticeInstance = new $classname(
                                $key,
                                $notice['id_shop_group'],
                                $notice['id_shop'],
                                $value
                            );
                        } else {
                ...
                if ($noticeInstance->controller === null || $noticeInstance->controller === $controller) {
                    $noticeInstances[] = $noticeInstance;
                }
            }
        }
    ...
    /**
     * Render current page notices instance
     *
     * @return string $notices
     */
    public static function renderNotices($controller)
    {
        $notices = self::getNoticeInstances(get_class($controller));

        $result = '';
        foreach ($notices as $notice) {
            $result .= $notice->render();
        }
     ...
     * @void
     */
    public static function addNotice($type, $shopGroupId, $shopId, $args = [])
    {
        if (!in_array($type, self::$coreNotices, true)) {
            $key = uniqid('lp_', false);
        } else {
            $key = $type;
        }

        $value = EncodeUtil::encode($args);

        // phpcs:ignore Generic.Files.LineLength
        $sql = 'INSERT IGNORE INTO `' . _DB_PREFIX_ . 'lp_notices` (`id_shop_group`, `id_shop`, `key`, `value`)
            VALUES (';
    ...
    {
        $sql = 'DELETE IGNORE FROM `' . _DB_PREFIX_ . 'lp_notices`
                WHERE ';
        if (null === $shopGroupId) {
    ...
    public static function removeAllNotices()
    {
        \Db::getInstance()->execute(
            'DELETE FROM `' . _DB_PREFIX_ . 'lp_notices`;'
        );
    }
